...
export default async function endpointCommand (project: Project, args: string[], options: any) {
    ...
    const filePath = join("lib/ipc/endpoints", path, endpoint + ".ipc-endpoint.ts");

    await project.generateFileFromTemplate("ipc:endpoint.ts", filePath, {
        name: endpoint
    });

    await insertTagLine(
        project.subPath(join("lib/ipc/endpoints", path, "index.ts")),
        "Imports",
        `import ${toVariableName(endpoint)} from "./${endpoint}.ipc-endpoint";`
    );
